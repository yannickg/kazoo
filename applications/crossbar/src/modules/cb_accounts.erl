...
validate(Context, ?AUTH_ACCOUNT_PLACEHOLDER) ->
...
validate(Context, ?AUTH_ACCOUNT_PLACEHOLDER, PathToken) ->
    validate(Context, cb_context:auth_account_id(Context), PathToken);
...
load_account_db(Context, [AccountId|_]) ->
    load_account_db(Context, AccountId);
load_account_db(Context, ?AUTH_ACCOUNT_PLACEHOLDER) ->
    lager:info("auth account placeholder used, deferring loading db until auth token is parsed"),
